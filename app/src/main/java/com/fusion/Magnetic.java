...
public class Magnetic extends Types {
    ...
    static void fInitMagCalibration(MagCalibration pthisMagCal,  MagneticBuffer pthisMagBuffer)
    {
        ...
        pthisMagCal.iMagCalHasRun = false;
        ...
        {
            pthisMagBuffer.tanarray[j] = (int) (100.0F * Math.tan(PI * (-0.5F + (float) (j + 1) / MAGBUFFSIZEX)));
        }
...
// the uncalibrated measurements iBs are stored in the buffer but the calibrated measurements iBc are used for indexing.
    ...
    static void fUpdateCalibration4INV(MagCalibration pthisMagCal, MagneticBuffer pthisMagBuffer, MagSensor pthisMag)
    {
        ...
        float pfRows[][];
        int iColInd[] = new int[4];
        int iRowInd[] = new int[4];
        ...
        pthisMagCal.ftrB = (float) Math.sqrt(pthisMagCal.fvecA[3] + pthisMagCal.ftrV[CHX] * pthisMagCal.ftrV[CHX] +
        ...
        pthisMagCal.ftrFitErrorpc = (float) Math.sqrt(fE / (float) pthisMagBuffer.iMagBufferCount) * 100.0F /
    ...
    static void fUpdateCalibration7EIG(MagCalibration pthisMagCal, MagneticBuffer pthisMagBuffer, MagSensor pthisMag)
    {
        ...
        Matrix.eigencompute10(pthisMagCal.fmatA, pthisMagCal.fvecA, pthisMagCal.fmatB, 7);
        ...
        Matrix.f3x3matrixAeqScalar(pthisMagCal.fA, 0.0F);
        det = 1.0F;
        for (l = CHX; l <= CHZ; l++)
        {
        ...
        if (det < 0.0F)
        {
            Matrix.f3x3matrixAeqMinusA(pthisMagCal.fA);
        ...
        pthisMagCal.ftrFitErrorpc = 50.0F * (float) Math.sqrt(Math.abs(pthisMagCal.fvecA[j]) / (float) pthisMagBuffer.iMagBufferCount) / Math.abs(ftmp);

        // normalize the ellipsoid matrix A to unit determinant
        Matrix.f3x3matrixAeqAxScalar(pthisMagCal.fA, (float) Math.pow(det, -(ONETHIRD)));

        // convert the geomagnetic field strength B into uT for normalized soft iron matrix A and normalize
        pthisMagCal.ftrB = (float) Math.sqrt(Math.abs(ftmp))* DEFAULTB  * (float) Math.pow(det, -(ONESIXTH));
    ...
    static void fUpdateCalibration10EIG(MagCalibration pthisMagCal, MagneticBuffer pthisMagBuffer, MagSensor pthisMag)
    {
        ...
        Matrix.eigencompute10(pthisMagCal.fmatA, pthisMagCal.fvecA, pthisMagCal.fmatB, 10);
        ...
        det = Matrix.f3x3matrixDetA(pthisMagCal.fA);
        if (det < 0.0F)
        {
            Matrix.f3x3matrixAeqMinusA(pthisMagCal.fA);
            pthisMagCal.fmatB[6][j] = -pthisMagCal.fmatB[6][j];
        ...
        Matrix.f3x3matrixAeqInvSymB(pthisMagCal.finvA, pthisMagCal.fA);
        ...
        pthisMagCal.ftrB = (float) Math.sqrt(Math.abs(pthisMagCal.fA[0][0] * pthisMagCal.ftrV[CHX] * pthisMagCal.ftrV[CHX] +
        ...
        // calculate the trial normalized fit error as a percentage
        pthisMagCal.ftrFitErrorpc = 50.0F * (float) Math.sqrt(Math.abs(pthisMagCal.fvecA[j]) / (float) pthisMagBuffer.iMagBufferCount) /
        ...
        Matrix.f3x3matrixAeqAxScalar(pthisMagCal.fA, (float) Math.pow(det, -(ONETHIRD)));
        pthisMagCal.ftrB *= Math.pow(det, -(ONESIXTH));
        ...
        Matrix.eigencompute10(pthisMagCal.fmatA, pthisMagCal.fvecA, pthisMagCal.fmatB, 3);
        ...
        {
            ftmp = (float) Math.sqrt(Math.sqrt(Math.abs(pthisMagCal.fvecA[j])));
